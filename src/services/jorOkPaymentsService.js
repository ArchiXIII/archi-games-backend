...
const { OkCallbackError, createOkCallbackSignature, CALLBACK_SIGNATURE_ERROR, CALLBACK_INVALID_PAYMENT } = require('./okPaymentsService');
...
class JorOkPaymentsService {
  ...
  async process(params) {
    if (!this.config.jorOkAppKey || !this.config.jorVkAppSecret) {
    ...
    if (!/^[a-f0-9]{32}$/.test(received) ||
        !safeEqual(createOkCallbackSignature(source, this.config.jorVkAppSecret), received)) {
