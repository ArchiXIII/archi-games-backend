...
Для Жора используются `JOR_VK_APP_ID`, `JOR_VK_APP_SECRET`, `JOR_VK_SERVICE_TOKEN`, `JOR_OK_APP_ID` и `JOR_OK_APP_KEY`. Связанное приложение OK использует тот же защищённый ключ и VK app ID, поэтому отдельные дубли этих параметров не нужны.
...
Миграция `009_jor_purchases.sql` создаёт таблицу и один индекс пользователя, не изменяя таблицы других игр. VK использует callback `/v1/vk/jor/payments/callback`, OK — `/v1/ok/jor/payments/callback`.
...
| `JOR_OK_APP_KEY` | публичный ключ приложения Жора в Одноклассниках для проверки callback |
